              ...
              <div class="col-sm-4">
                ...
                <div class="form-group">
                  <label for="customFile">或 上傳圖片
                    <i class="fas fa-spinner fa-spin" v-if="status.fileUploading"></i> <!-- 上傳效果 -->
                  </label>
...
export default {
  ...
  methods: {
    getProducts(page = 1) {
      // const api = 'https://vue-course-api.hexschool.io/api/ningningccn/products'
    ...
    openModal(isNew , item ){
      if(isNew) {
        ...
        this.tempProduct = Object.assign({},item); // es6 方法將item 物件傳入 避免有參考特性
        this.isNew =false ;
      }
    ...
    updateProduct() {
      ...
      this.$http[httpMethod](api,{data:vm.tempProduct}).then((response) => {
        ...
        if (response.data.success) {
          $('#productModal').modal('hide'); //將model視窗關閉
          vm.isLoading = true;
          vm.getProducts();
        }else{
    ...
    uploadFile() {
      ...
      this.$http.post(url,formData,{
        ...
        if(response.data.success) {
          ...
          vm.$set(vm.tempProduct,'imageUrl', response.data.imageUrl) //強制 把（位置）（欄位）（上傳的連結）
        }else {
